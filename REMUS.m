...
% - Hydrostatic terms are disabled. This represents neutral buoyancy with no
%   modeled restoring moment from a center-of-buoyancy/center-of-gravity offset.
% - The supplied mass and inertia values are treated as assembled-vehicle
%   properties about the body-frame origin. CMG module mass, transverse
%   inertia, mounting orientation, and parallel-axis shifts are not added here.
% - The vehicle inertia tensor is presently diagonal (Ix, Iy, Iz), an
%   assumption that should be revisited if detailed mass properties are added.
%
% Inputs
...
% - tau_cmg1, tau_cmg2: Body-axis reaction couples generated by CMG modules
%                       #1 and #2. The indices do not encode mounting location.
% - thruster: Fixed lateral-thruster generalized force and actuator data.
% 
% Outputs
...
% Control parameters for CMG modules #1 and #2. These numbers identify the
% modules only; the present model has no aft/forward position vectors.
alphadot1 = contpar.alphadot1;  % CMG #1 gimbal angular velocity
Omegadot1 = contpar.Omegadot1;  % CMG #1 flywheel angular acceleration
alphadot2 = contpar.alphadot2;  % CMG #2 gimbal angular velocity
Omegadot2 = contpar.Omegadot2;  % CMG #2 flywheel angular acceleration
...
% Assembled-vehicle inertial properties supplied about the body-frame origin.
% No CMG parallel-axis correction is performed inside this dynamics function.
% Such a correction belongs in Ix/Iy/Iz (or a future full inertia tensor), not
% as an Omega-dependent forcing term in K, M, or N.
...
% Moments
Kpp         = params.rollDragCoefficient; % Roll drag (kg m^2/rad^2)
Mww         = 0;                % 3.18;     (kg)
Mqq         = -9.4;             %           (kgm^2/rad^2)
Muw         = 0;                % 24.0;     (kg)
Mwdot       = 0;                % -1.93;    (kgm)
...
% Hydrostatic forces and restoring moments are disabled. Consequently, CB
% coordinates are not defined and any CB-CG separation has no modeled effect.
XHS         = 0;                % -(W-B)*sin(theta);
...
Y           = YHS + Yhyd + tauC.YD + thruster.Y;                % Total sway force      (N)
Z           = ZHS + Zhyd + tauC.ZD;                             % Total heave force     (N)
% tauC.KD/MD/ND are allocator requests, not independent external moments.
% Only the moments actually generated by the CMGs act on the vehicle. These
% are pure couples and may be summed directly regardless of mounting position;
% an r-cross-F term would apply to an off-origin force, not to a pure couple.
% KExternalDump is the explicit external reaction used for CMG momentum
% unloading. It is not generated by the CMG and must ultimately be assigned
% to a thruster, control surface, or another environmental interaction.
K           = KHS + Khyd + tau_cmg1.K + tau_cmg2.K ...
              + tauC.KExternalDump + tauC.KDisturbance;          % Total roll moment (N*m)
M           = MHS + Mhyd + tau_cmg1.M + tau_cmg2.M;              % Total pitch moment    (N*m)
N           = NHS + Nhyd + tau_cmg1.N + tau_cmg2.N + thruster.N; % Total yaw moment      (N*m)
...
% With diagonal inertia and zero CG offsets, roll plus the unavoidable CMG
% yaw reaction produces a pitch-equation term (Iz-Ix)*p*r. Thus a nonzero
% pitch response can occur even when the allocator holds applied CMG M near
% zero. Likewise, (Ix-Iy)*p*q appears in yaw. These are rigid-body coupling
% terms, not allocator error or evidence of an additional CMG pitch command.
...
% Combine results into the generalized position derivative vector. States
% 17-18 are physical gimbal rates; their derivatives are the rate-servo
% accelerations calculated in CONTROL.
Etadot      = [etadot;nudot;alphadot1;Omegadot1;alphadot2;Omegadot2; ...
               contpar.gimbalAccel1;contpar.gimbalAccel2];
