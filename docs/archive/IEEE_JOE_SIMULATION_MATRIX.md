...
**Historical planning document.** First-draft scope, retained cases, and
criteria are now frozen in `IEEE_JOE_FIRST_DRAFT_FREEZE.md` (2026-09-22).
Pending-status statements and next-task lists below are preserved as history,
not the current work queue. In particular, fixed-time ablation is deferred.
